...
**Etap 0 — Fundament** (1 sesja): `Investment` (bez list relacji!) + seed „Nova Staffa" + permissions `budowa`/`checkin` + `PREFERRED_LANDING_ORDER` + workspace „Budowa" + szkielet dashboardu. Deploy: `db push` + nadanie uprawnień + re-login.

**Etap 1 — Komunikacja z budowy** (1–2 sesje): `SiteReport` (bez taskUpdates) + `SitePhoto` + `ConstructionComment` + route group `(mobile)` + **`/checkin` lite** (co zrobiono / problem / decyzja / reakcja wykonawcy / zdjęcia) + pipeline zdjęć + PWA manifest + wydłużona sesja kierownika + mobilne logowanie + `/budowa/dziennik` (feed+galeria) + **Widok Prezesa** (mobile-first) + reguły `BUDOWA_PROBLEM`/`BUDOWA_RAPORT_DECYZJA`/`BUDOWA_WYKONAWCA`/`BUDOWA_WYJASNIENIE` (+ gałęzie reconcile) + konto kierownika + przełącznik widoku w `/profil`. **Wartość dla 3 z 4 użytkowników w pierwszym tygodniu — bez czekania na Gantt.**
