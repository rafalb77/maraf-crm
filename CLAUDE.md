...
- **`docs/incident-bogdan-mail-status.md`** — 🟡 **OTWARTE** (po stronie usera, nie kod): incydent hack SMTP `bogdan.boruch@maraf.pl` (Synthient credential stuffing) + spam przez zapomnianego WordPressa na `maraf.home.pl/autoinstalator/wordpress`. Hasło zmienione, ale Bogdan musi wymienić hasło wszędzie indziej + skan komputera + WordPress cleanup. Plik ma listę TODO i lessons learned.
