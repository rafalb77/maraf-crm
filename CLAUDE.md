...
- `RESERVATIONS_CRON_SECRET` — sekret dla cron-a wysyłającego codzienny digest o rezerwacjach miękkich wygasających w ciągu 48h. Wymagany przez `POST /api/public/reservations/expiring-email?secret=...` (lub Bearer). Generuj jak wyżej. Coolify scheduled task: np. codziennie rano `0 8 * * *`. Odbiorca = `Settings.reservationsAlertEmail` (zarządzany w `/settings`), fallback `NEXT_PUBLIC_ADMIN_EMAIL`. Patrz moduł `/rezerwacje`.
