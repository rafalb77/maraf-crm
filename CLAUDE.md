...
- **`docs/rezerwacje-powiadomienia-decyzje.md`** — automatyczne powiadomienia przed wygaśnięciem rezerwacji miękkiej: e-mail/SMS do klienta (X godzin przed, default 48) + zadanie „Zadzwoń" na pulpicie. `lib/reservation-alerts.ts`, `lib/sms.ts` (SMSAPI.pl), model `NotificationLog` (idempotencja), cron `/api/public/reservations/alerts` co 15 min, sekcja w `/settings`. Deploy checklist w pliku.
- **`docs/sprawy-decyzje.md`** — moduł Sprawy (`/cases`): repozytorium spraw (reklamacje rękojmia + urzędowe) z osią korespondencji (pisma/skany), terminami ustawowymi + przypomnieniami (cron `CASES_CRON_SECRET`) i OCR skanów (Tesseract natywny). Model Case/CaseEntry/CaseDocument, sygnatura `REK/2026/0042`, deploy (wymaga `prisma db push` + **rebuild** + scheduled tasks + permission `cases`), otwarte kierunki. Osobny moduł obok Serwisu.
...
- **`docs/rozliczenia-powiernicze-decyzje.md`** — moduł **Rozliczenia powiernicze** (`/finanse/powiernicze`, MD-only). Kontrola wpłat nabywców z harmonogramu umowy deweloperskiej (`ContractPayment`) wobec wyciągu z ING Banku Śląskiego. Import **MT940 / CSV / camt.053** (parsery w `lib/bank-import/`, specyfika ING zweryfikowana z dok. banku), auto-dopasowanie (`lib/bank-reconcile.ts`: subrachunek OMRP→nr umowy→nazwisko→kwota), **odsetki ustawowe za opóźnienie metodą okresową** (`lib/interest.ts`, tabela stawek 2020–2026 — AKTUALIZOWAĆ przy zmianach stopy NBP), rejestr wpłat + rejestr odsetek + alerty. Zbudowany 2026-07-14. **WYMAGA `prisma db push` + rebuild** (3 nowe tabele + kolumny). Buduje na etapie 2 Finansowania.
...
- **`docs/odbiory-decyzje.md`** — moduł **Odbiory** (`/odbiory`, permission `odbiory`, workspace Budowa): odbiory robót od wykonawców — usterki jako pinezki na rzutach kondygnacji (widok terenowy `(mobile)/odbiory/teren/[id]`, offline-first przez IndexedDB + outbox), słownik/legenda, pakiety wykonawców po prywatnym linku `/w/[token]` (bez logowania), tryb ponownego odbioru, protokół do druku `(print)/odbiory/[id]/protokol`, AI porządkujące dyktowany opis (`api/odbiory/ai/parse`). MVP z 2026-09-12; deploy wymaga `prisma db push` (9 nowych tabel, istniejące nietknięte) + uprawnienie `odbiory`.
...
- **`docs/budowa-rozpoczecie.md`** — 🟢 **ZAAKCEPTOWANA, W REALIZACJI** (koncepcja 2026-07-08/09, 9 decyzji Rafała podjętych 2026-07-09 — sekcja „Decyzje podjęte"): moduł **Budowa (Project Manager)** — trzeci workspace: Gantt (SVAR React Gantt MIT ze spikem, fallback DHTMLX), mobilny check-in kierownika (`/checkin`, route group `(mobile)`, permission `checkin`, sesja 30 dni), Widok Prezesa dla Bohdana (mobile-first), dziennik budowy + zdjęcia, most do Finansów (FK `PurchaseInvoice.investmentId/StageId/TaskId` + mostek `Subcontractor.vendorId`→`Vendor`). Nowe modele: `Investment`, `ConstructionStage/Task`, `TaskDependency`, `SiteReport(+TaskUpdate)`, `SitePhoto`, `ConstructionComment` (+`ConstructionRisk` slim w E4). Fazowanie: E0 fundament (✅ prod 2026-07-09) → E1 komunikacja (✅ kod 2026-07-09: /checkin + dziennik + Widok Prezesa + reguły BUDOWA_* + sesja 30d checkin-only) → E2 harmonogram (import z Excela Rafała) → E3 most finansowy → E4 (digest tygodniowy do wszystkich + rejestr ryzyk potwierdzone). Deploy E1 wymaga `db push` (tabele SiteReport/SitePhoto/ConstructionComment) — **cały zespół musi się raz przelogować** (egzekwowanie sesji po token.authAt).
- ⚠️ **Przeroby — sygnał od Rafała (2026-07-09): moduł „do poprawienia albo usunięcia — nie odpowiada mi".** Nie budować nowych funkcji NA WIDOKACH Przerobów bez rozmowy o kierunku; modele danych `Subcontractor`/`SubContract`/`Protocol` zostają (Budowa na nich polega). Szczegóły: `docs/budowa-rozpoczecie.md` sekcja Pułapki.
...
- **`docs/incident-utrata-danych-klientow.md`** — 🟡 **OTWARTE** (dane do odtworzenia): `scripts/dedupe-clients.js` gubił pola kontaktowe (telefon/adres) kasując duplikaty bez przeniesienia skalarów na keepera. **Kod naprawiony 2026-07-12** (scala puste pola przed delete). Utracone dane = hard delete → odtworzenie tylko z backupu OVH. NIE zgadywać wartości. Przed każdym `dedupe-clients.js --apply`: zrobić backup + przejrzeć raport (dry-run pokazuje `→ scali:`).
- **`docs/dostarczalnosc-maili-maraf-status.md`** — 🟢 **ZAMKNIĘTE 2026-07-11** (Rafał: maile dochodzą, temat odpuszczony). maraf.pl ma komplet (SPF `~all`+include, DMARC, DKIM, blacklisty czyste). novastaffa.pl (nadawca CRM) formalnie bez jawnego DKIM/DMARC przez wildcard DNS, ale maile docierają — checklist naprawy zostaje w pliku na wypadek realnych odrzuceń/spamu w przyszłości. Nie wracać bez sygnału.
...
- `RESERVATIONS_CRON_SECRET` — sekret dla DWÓCH cronów rezerwacji: (1) dzienny digest do handlowca `POST /api/public/reservations/expiring-email` (Coolify: `0 8 * * *`, odbiorca = `Settings.reservationsAlertEmail`, fallback `NEXT_PUBLIC_ADMIN_EMAIL`); (2) powiadomienia do KLIENTÓW przed wygaśnięciem miękkiej `POST /api/public/reservations/alerts` (Coolify: `*/15 * * * *`, e-mail/SMS + zadanie „Zadzwoń" na pulpicie, konfiguracja w `/settings`). Generuj jak wyżej. **Pułapka: obraz produkcyjny nie ma curla** — komendy scheduled tasków przez `node -e "fetch(...)"` (gotowce w `docs/rezerwacje-powiadomienia-decyzje.md`).
