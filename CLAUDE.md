...
- **`docs/outlook-rafal-recreate-rozpoczecie.md`** — 🟡 **OTWARTE** (operacyjne, do zrobienia jutro w biurze): Outlook Rafała na DWÓCH komputerach nie chce zaakceptować nowego hasła `rafal.boruch@maraf.pl` (zmienionego profilaktycznie 2026-05-19 po incydencie Bogdana). Webmail wpuszcza → hasło OK, problem to blokada IMAP brute-force po stronie home.pl (oba Outlooki bombardują starym hasłem). Plan: eksport reguł `.rwz` → backup PST → recreate konta z nowym hasłem → import reguł — na obu kompach. Plik ma pełną procedurę krok po kroku.
