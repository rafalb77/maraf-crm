...
- **`docs/dostarczalnosc-maili-maraf-status.md`** — 🟡 **OTWARTE**: dostarczalność maili. **maraf.pl NAPRAWIONE** (2026-07-11: SPF z `~all`+include ✅, DMARC ✅, DKIM podpisuje ✅, blacklisty czyste; zostaje ewent. ticket PTR/reputacja jeśli odrzucenia nadal występują). **Czeka novastaffa.pl** — nadawca CRM (`biuro@novastaffa.pl`, m.in. automatyczne powiadomienia o rezerwacjach do klientów): wildcard `*` CNAME w strefie zjada rekordy → **brak DKIM i brak DMARC** (SPF pass). Checklist naprawy (panel home.pl: włączyć DKIM, dodać `_dmarc` TXT, test mail-tester przez przycisk „Test e-mail" w /settings) — w pliku, sekcja „Update 2026-07-11".
