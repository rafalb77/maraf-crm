...
- **`docs/finanse-finansowanie-etap2-rozpoczecie.md`** — 🟡 **OTWARTE**: ETAP 2 modułu Finansowanie inwestycji — auto-tworzenie `EscrowDeposit` przy rejestracji wpłaty nabywcy w module Sprzedaż. Bloker: Sprzedaż (`Contract`) nie ma modelu wpłat — trzeba najpierw dodać `ContractPayment` + UI + trigger. Etap 1 (kredyty + escrow OMRP + zwroty VAT, ręczne wpisywanie, cashflow gotówkowy + DSCR) wdrożony 2026-06-03 commit `334a0f6` — patrz `docs/finanse-decyzje.md`. **WYMAGA na produkcji `prisma db push`** (6 nowych tabel: Loan/LoanTranche/LoanRepayment/VatRefund/EscrowAccount/EscrowDeposit/EscrowRelease).
