...
- `RESERVATIONS_CRON_SECRET` — sekret dla DWÓCH cronów rezerwacji: (1) dzienny digest do handlowca `POST /api/public/reservations/expiring-email` (Coolify: `0 8 * * *`, odbiorca = `Settings.reservationsAlertEmail`, fallback `NEXT_PUBLIC_ADMIN_EMAIL`); (2) powiadomienia do KLIENTÓW przed wygaśnięciem miękkiej `POST /api/public/reservations/alerts` (Coolify: `*/15 * * * *`, e-mail/SMS + zadanie „Zadzwoń" na pulpicie, konfiguracja w `/settings`). Generuj jak wyżej. Patrz `docs/rezerwacje-powiadomienia-decyzje.md`.
