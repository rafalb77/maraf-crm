...
export async function GET(_req: NextRequest) {
  ...
  const images = await prisma.investmentImage.findMany({
...
export async function POST(req: NextRequest) {
  ...
  if (!isAdmin(session.user?.email)) {
    return NextResponse.json({ error: 'Tylko admin moze zarzadzac wizualizacjami inwestycji' }, { status: 403 })
  }

  const formData = await req.formData()
  const files = formData.getAll('files').filter((f): f is File => f instanceof File)
  const kind = (formData.get('kind') as string) || 'INNE'
  ...
  const uploadsDir = path.join(process.cwd(), 'public', 'uploads', 'investment')
  await mkdir(uploadsDir, { recursive: true })

  const existingCount = await prisma.investmentImage.count()
  const hasPrimary = (await prisma.investmentImage.count({ where: { isPrimary: true } })) > 0
  ...
  for (const file of files) {
    ...
    const url = `/uploads/investment/${filename}`
    const isPrimary = !hasPrimary && idx === 0 && existingCount === 0
    const image = await prisma.investmentImage.create({
      data: {
        url,
